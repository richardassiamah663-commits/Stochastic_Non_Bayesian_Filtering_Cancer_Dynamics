...
# =============================================================================
def propagate_particle(x, k, rng):
    """Euler-Maruyama step for one particle."""
...
def log_likelihood(z, x):
    """Gaussian log-likelihood of observation z given state x."""
    y_pred = H @ x
    diff = z - y_pred
    return -0.5 * np.sum((diff / meas_noise_std)**2)
...
def run_pf(seed):
    ...
    for k in range(1, T):
        # Propagate particles
        for j in range(N_particles):
            particles[j] = propagate_particle(particles[j], k-1, rng)
        ...
        for j in range(N_particles):
            log_w[j] = log_likelihood(z, particles[j])
